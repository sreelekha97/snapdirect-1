...
public class ContactsRetrieverTask extends AsyncTask<String, Void,
    ...
    protected void onPostExecute(final ArrayList<ContactEntry> contacts) {
        ...
        for (int i = 0; i < contacts.size(); i++) {
            if (Snapdirect.getFriendsDataSource().
                ...
                Snapdirect.getFriendsDataSource().createFriend(contacts.get(i).strName,
        ...
        ServerInterface.postContactsRequest(Snapdirect.getMainActivity(), hContactKeys,
                ...
                public void onResponse(Map<String, UserProfile> response) {
                    for (int i = 0; i < hContactKeys.size(); i++) {
                        if (response.containsKey(hContactKeys.get(i))) {
                            FriendEntry friend = Snapdirect.getFriendsDataSource().
                            ...
                            if (friend != null) {
                                ...
                                Snapdirect.getFriendsDataSource().updateFriend(friend);
                            }
                        }
                    }
                    ArrayList<FriendEntry> friendsList = Snapdirect.getFriendsDataSource().
    ...
    public ArrayList<ContactEntry> getContacts() {
        ...
        MainActivity mainActivity = Snapdirect.getMainActivity();
