...
public class GalleryAdapter extends ArrayAdapter<ImageEntry> {
    ...
    private String getUnreadCommentCount (String strImageID) {
        String strCount = "";
        if (Snapdirect.getPreferences().unreadImagesMap.containsKey(strImageID) &&
                Snapdirect.getPreferences().unreadImagesMap.get(strImageID)>0) {
            int intCommentCount = Snapdirect.getPreferences().
                    unreadImagesMap.get(strImageID);
            if (intCommentCount < 10)
                strCount = Snapdirect.getPreferences().unreadImagesMap.get(strImageID).toString();
            else
                strCount = ">9";
        }
        return strCount;
