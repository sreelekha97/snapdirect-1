...
public class ImageDownloader extends AsyncTask<String,Integer,Long> {
    ...
    public ImageDownloader(String strURL, File destFile, Boolean boolShowProgress,
        ...
        if (boolShowProgress)
            mProgressDialog = new ProgressDialog(Snapdirect.getImageDetailsActivity());
    }
    ...
    protected void onPreExecute() {
        super.onPreExecute();
        if (boolShowProgress) {
            mProgressDialog.setMessage(Snapdirect.getImageDetailsActivity().
    ...
    protected void onProgressUpdate(Integer... progress) {
        if (boolShowProgress) {
            ...
            if (mProgressDialog.getProgress() == mProgressDialog.getMax()) {
                mProgressDialog.dismiss();
                Toast.makeText(Snapdirect.getImageDetailsActivity(), Snapdirect.getImageDetailsActivity().
    ...
    protected void onPostExecute(Long l) {
        ...
        if (!strFilename.isEmpty())
            try {
                MediaScannerConnection.scanFile(Snapdirect.getImageDetailsActivity(),
