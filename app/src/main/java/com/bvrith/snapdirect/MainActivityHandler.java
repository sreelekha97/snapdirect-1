...
import com.bvrith.snapdirect.common.Preferences;
...
public class MainActivityHandler {
	...
	public void init(MainActivity activity) {
        ...
        Snapdirect.setMainActivity(activity);
        prefs = new Preferences(activity);
		if (!prefs.loadPreferences()) {
            Snapdirect.getPreferences().strUserID = "";
        ...
        if (Snapdirect.getPreferences().strPushRegID.isEmpty() &&
                !Snapdirect.getPreferences().strUserID.isEmpty())
            registerPushID();
	}
        ...
        ServerInterface.getServerInfoRequest(activity,
                new Response.Listener<HashMap<String, String>>() {
                    ...
                    public void onResponse(HashMap<String, String> response) {
                        ...
                        if (Snapdirect.getPreferences().strServerInfo.equals(strServerInfo)) return;
        ...
        alert.setPositiveButton(R.string.alert_ok_button,
            new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int whichButton) {
                    ...
                    Gson gson = new Gson();
                    Snapdirect.getPreferences().strServerInfo = gson.toJson(serverInfo);
                ...
                if (!strPushID.isEmpty()) {
                    ...
                    ServerInterface.updateProfileRequest(activity, profile,
                            new Response.Listener<String>() {
                                @Override
                                public void onResponse(String response) {
                                    Snapdirect.getPreferences().strPushRegID = strPushID;
                                    Snapdirect.getPreferences().savePreferences();
    ...
    private boolean checkPlayServices() {
        ...
        if (resultCode != ConnectionResult.SUCCESS) {
            if (GooglePlayServicesUtil.isUserRecoverableError(resultCode)) {
                GooglePlayServicesUtil.getErrorDialog(resultCode, Snapdirect.getMainActivity(),
        ...
        if ((i.getIntExtra("event_type", 0) == Constants.EVENT_NEW_COMMENT) &&
            ...
            try {
                ...
                if (dataJson.has(Constants.KEY_IMAGEID)) {
                    Intent mIntent = new Intent(Snapdirect.getMainActivity(),
                            ImageDetailsActivity.class);
                    String strImageId = dataJson.getString(Constants.KEY_IMAGEID);
                    ImageEntry imageEntry = Snapdirect.getImagesDataSource().
                            getImageByServerID(strImageId);

                    if (imageEntry != null) {
                        Snapdirect.setGalleryImageDetails(imageEntry);
                        Bundle b = new Bundle();
                        ...
                        try {
                            ...
                            Snapdirect.setImageDetails(imageData);
                        } catch (Exception e) {
                    ...
                    Snapdirect.getMainActivity().startActivity(mIntent);
                }
            ...
            try {
                ...
                if ((dataJson.has(Constants.KEY_IMAGE)) &&
                    ...
                    Snapdirect.setImageDetails(feedEntry);
                    Intent mIntent = new Intent(Snapdirect.getMainActivity(),
                            ImageDetailsActivity.class);
                                            ...
                                            selectedChannels, new Response.Listener<String>() {
                                                ...
                                                public void onResponse(String s) {
                                                    for (int i = 0; i < selectedChannels.size(); i++) {
                                                        FriendEntry channel = Snapdirect.
                                                        ...
                                                        if (channel != null) {
                                                            channel.setStatus(FriendEntry.
                                                                    INT_STATUS_FRIEND);
                                                            Snapdirect.getFriendsDataSource().
                                                                    updateFriend(channel);
                                                        }
                                                    }
                                                    Snapdirect.getFriendsFragmentTab().syncChannels();
                                                }}, null);
                                    }
                                })
    ...
    public void syncAvatar(){
        ...
        if (!avatar.exists())
            ServerInterface.getUserProfileRequest(activity,
                new String[]{Snapdirect.getPreferences().intPublicID.toString()},
                new Response.Listener<HashMap<String, UserProfile>>() {
                    ...
                    public void onResponse(HashMap<String, UserProfile> stringUserProfileHashMap) {
                        ...
                        UserProfile profile = stringUserProfileHashMap.get(
                                Snapdirect.getPreferences().intPublicID.toString());
